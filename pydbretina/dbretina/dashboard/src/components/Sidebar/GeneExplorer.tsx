...
export default function GeneExplorer() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      const [stats, groups] = await Promise.all([
        fetchGeneStatistics(debouncedQuery),
        fetchGeneGroups(debouncedQuery, 50),
      ]);
  ...
  const handleHighlightAll = useCallback(() => {
    ...
    // Find node IDs for groups containing this gene
      ...
      {expanded && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div>
            ...
            {error && (
              <div style={{ fontSize: 11, color: "#e74c3c", marginTop: 4 }}>{error}</div>
            )}
          </div>
          ...
          {!stats && !error && (
            <div style={{ fontSize: 11, color: "var(--text-secondary)" }}>
              Enter a gene name (e.g., APOE, TP53) to see which diseases/groups contain it and
